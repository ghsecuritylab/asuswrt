...
static void do_detwan_cgi(char *url, FILE *stream)
{
	...
	if (!strcmp(action_mode, "GetWanStatus")) {
		...
		if(!strcmp("4", nvram_safe_get("autodet_state"))) state = 0;

		if(!strcmp("6", nvram_safe_get("autodet_state")) || !strcmp("6", nvram_safe_get("autodet_auxstate"))){
			if(
				!strcmp("2", nvram_safe_get("wan0_state_t")) && 
				!strcmp("0", nvram_safe_get("wan0_sbstate_t")) && 
				!strcmp("0", nvram_safe_get("wan0_auxstate_t")) &&
				(!strcmp("2", nvram_safe_get("link_internet")) || !strcmp("0", nvram_safe_get("wan0_realip_state")) || !strcmp("0", nvram_safe_get("wan1_realip_state")))
			){
				state = 2;
			}
			else{
		...
		if(!strcmp("2", nvram_safe_get("wan0_state_t")) && !strcmp("0", nvram_safe_get("wan0_sbstate_t")) && !strcmp("0", nvram_safe_get("wan0_auxstate_t"))){
			state = 2;
		}

		if(!strcmp("4", nvram_safe_get("wan0_state_t")) && !strcmp("4", nvram_safe_get("wan0_sbstate_t")) && !strcmp("0", nvram_safe_get("wan0_auxstate_t"))){
			state = 7;
		}
...
do_get_IFTTTtoken_cgi(char *url, FILE *stream)
{
	...
	stoken = websGetVar(wp, "shortToken","");
	 ...
	 ret = gen_IFTTTtoken(stoken, pincode);
